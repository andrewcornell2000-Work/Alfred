...
- **Environment:** `.env` file manages `OPENAI_API_KEY` and `ANTHROPIC_API_KEY`; virtual environment `.venv` used  
...
6. **Memory consolidation:** After â‰¥10 log entries, `consolidate_memory_if_needed()` uses GPT-4.1-mini to merge logs into `memory/session-summary.md`, retaining last 5 logs  
