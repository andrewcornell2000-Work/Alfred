...
11. Security hardening — private/credential files untracked from git; API token input masked in terminal
...
- **Codex TTY error:** `Error: stdout is not a terminal` on all headless Codex invocation paths — PTY wrapper or alternative invocation needed
